...
<table id="example" class="table table-striped table-bordered text-center">
        ...
        <c:set var="count" value="${count + 1}" scope="page"></c:set>
            <tr>
                <td>${count}</td>
                <td>${product.name}</td>
                <td><img src="${pageContext.request.contextPath}/product/display/${product.id}" style="height:100px"/></td>
                <td>${product.description}</td>
                <td>${product.price}</td>
                <td><fmt:formatDate pattern="dd-MMM-yyyy" value="${product.createdDate}" /></td>
